"""
    curvature(dx::AbstractVector, A::AbstractArray)

Calculate ∇×A where `A` is a 4D array of size (3, nx, ny, nz) and `dx` is the grid interval.
"""
function curvature(dx::AbstractVector, A::AbstractArray{T,N}) where {T,N}
   @assert N == 4 "Input vector shall be organized by 3D Cartesian indexes!"
   ...
   if any(==(1), size(A)) # 2D
      if size(A,4) == 1
         for j in 2:size(A,3)-1, i in 2:size(A,2)-1
      ...
      elseif size(A,3) == 1
         for k in 2:size(A,4)-1, i in 2:size(A,2)-1
      ...
      for k in 2:size(A,4)-1, j in 2:size(A,3)-1, i in 2:size(A,2)-1
