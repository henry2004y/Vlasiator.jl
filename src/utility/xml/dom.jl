...
"""
function locatefirst(xpath::AbstractString, attribute::AbstractString,
   value::AbstractString, node::Node)
   locatefirst(split(xpath, '/'), attribute, value, node)
end

function locatefirst(xpath::AbstractString, attribute::AbstractString,
   value::AbstractString, doc::Document)
   locatefirst(split(xpath, '/'), attribute, value, doc)
end

function locatefirst(xpath::Array{T}, attribute::AbstractString, value::AbstractString,
   doc::Document) where T <: AbstractString
   if isempty(xpath) || !hasroot(doc)
      return nothing
   end
...
function locatefirst(xpath::Array{T}, attribute::AbstractString, value::AbstractString,
   node::Node) where T <: AbstractString
