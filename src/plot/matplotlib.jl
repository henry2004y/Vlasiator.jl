# Plotting functionalities from Matplotlib through PythonCall.
...
@static if pyconvert(String, mpl.__version__) ≥ "3.3"
   mpl.rc("image", cmap="turbo") # set default colormap
end

@static if pyconvert(String, mpl.__version__) < "3.5"
   mpl.rc("pcolor", shading="nearest") # newer version default "auto"
end
...
"""
function plot(meta::MetaVLSV, var, ax=nothing; comp=0, kwargs...)
...
"""
function streamplot(meta::MetaVLSV, var::AbstractString, ax=nothing;
...
"""
function quiver(meta::MetaVLSV, var::AbstractString, ax=nothing;
...
"""
function pcolormesh(meta::MetaVLSV, var::AbstractString, ax=nothing;
   ...
   if colorscale != SymLog
      ...
      c = ax.pcolormesh(x1[range1], x2[range2], data[range2, range1];
         norm, cmap=mpl.cm.RdBu_r, kwargs...)
   end
...
For fine-grain control, it is suggested to use the norm methods from `mpl.colors`.
For instance,
```
julia> norm = mpl.colors.CenteredNorm(); # after matplotlib v3.4
julia> norm = mpl.colors.BoundaryNorm(boundaries=[0, 1], ncolors=2);
julia> norm = mpl.colors.TwoSlopeNorm(vmin=-500., vcenter=0, vmax=4000);
```
There are also various options for the ticks available in `mpl.ticker`.
"""
function set_colorbar(colorscale::ColorScale, v1, v2, data=[1.0f0])
   vmin, vmax = set_lim(Float32(v1), Float32(v2), data, colorscale)
   if colorscale == Linear
      levels = mpl.ticker.MaxNLocator(nbins=255).tick_values(vmin, vmax)
      norm = mpl.colors.BoundaryNorm(levels, ncolors=256, clip=true)
      ticks = mpl.ticker.LinearLocator(numticks=9)
   elseif colorscale == Log # logarithmic
      norm = mpl.colors.LogNorm(;vmin, vmax)
      ticks = mpl.ticker.LogLocator(base=10, subs=collect(0:9))
   else # symmetric log
      linthresh = 1.0
      logstep = 1
      ...
      norm = mpl.colors.SymLogNorm(;linthresh, linscale=0.03, vmin, vmax, base=10)
