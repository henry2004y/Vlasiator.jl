...
@static if pyconvert(String, matplotlib.__version__) ≥ "3.3"
   matplotlib.rc("image", cmap="turbo") # set default colormap
end

@static if pyconvert(String, matplotlib.__version__) < "3.5"
...
magnitude. Any valid keyword argument for `pyplot.plot` is accepted.

# Keywords
- `comp::Int`: the component of a vector, chosen from any valid integers.
"""
function PythonPlot.plot(meta::MetaVLSV, var::String, ax::Union{Py,Nothing}=nothing;
   comp::Int=0, kwargs...)
...
Any valid keyword argument for `pyplot.streamplot` is accepted.
"""
function PythonPlot.streamplot(meta::MetaVLSV, var::String, ax::Union{Py,Nothing}=nothing;
...
Any valid keyword argument for `pyplot.quiver` is accepted.
"""
function PythonPlot.quiver(meta::MetaVLSV, var::String, ax::Union{Py,Nothing}=nothing;
...
Any valid keyword argument for `pyplot.pcolormesh` is supported.
...
```
"""
function PythonPlot.pcolormesh(meta::MetaVLSV, var::String,
   ax::Union{Py,Nothing}=nothing; comp::Union{Int, Symbol}=0,
...
Any valid keyword argument for `pyplot.contour` is supported.
"""
function PythonPlot.contour(meta::MetaVLSV, var::String,
   ax::Union{Py,Nothing}=nothing; comp::Union{Int, Symbol}=0,
   axisunit::AxisUnit=EARTH, colorscale::ColorScale=Linear, addcolorbar::Bool=true,
   vmin::Float64=-Inf, vmax::Float64=Inf, extent::Vector{Float64}=[-Inf, Inf, -Inf, Inf],
   kwargs...)
...
Any valid keyword argument for `pyplot.contourf` is supported.
"""
function PythonPlot.contourf(meta::MetaVLSV, var::String,
   ax::Union{Py,Nothing}=nothing; comp::Union{Int, Symbol}=0,
   axisunit::AxisUnit=EARTH, colorscale::ColorScale=Linear, addcolorbar::Bool=true,
   vmin::Float64=-Inf, vmax::Float64=Inf, extent::Vector{Float64}=[-Inf, Inf, -Inf, Inf],
   kwargs...)
...
function _plot2d(f::Py, meta::MetaVLSV, var::String,
   ax::Union{Py,Nothing}=nothing; comp::Union{Int, Symbol}=0,
   axisunit::AxisUnit=EARTH, colorscale::ColorScale=Linear, addcolorbar::Bool=true,
   vmin::Float64=-Inf, vmax::Float64=Inf, extent::Vector{Float64}=[-Inf, Inf, -Inf, Inf],
   kwargs...)
...
function set_plot(c::Py, ax::Py, pArgs::PlotArgs, ticks, addcolorbar::Bool)
   (;str_title, strx, stry, cb_title) = pArgs

   if addcolorbar
      cb = pyplot.colorbar(c; ax, ticks, fraction=0.04, pad=0.02)
   ...
   for loc in ("left", "bottom", "right", "top")
      @static pyconvert(String, matplotlib.__version__) < "3.4" ?
...
function vdfslice(meta::MetaVLSV, location::AbstractVector,
   ax::Union{Py,Nothing}=nothing; limits::Vector{Float64}=[-Inf, Inf, -Inf, Inf],
   ...
   cb = pyplot.colorbar(h[4]; ax, fraction=0.04, pad=0.02)
...
"""
function plotmesh(meta::MetaVLSV, ax::Union{Py,Nothing}=nothing;
