...
for (i, fname) in enumerate(filenames)
   ...
   axs[1].plot(loc, rho_extract ./ 1e6, label="Proton density")
   axs[1].vlines(loc[imagnetopause_], ρmin, ρmax, colors="r", linestyle="dashed", alpha=0.5)

   axs[2].plot(loc, v_extract[1,:] ./ 1e3, label="Vx")
   axs[2].vlines(loc[imagnetopause_], vmin, vmax, colors="r", linestyle="dashed", alpha=0.5)

   axs[3].plot(loc, pdyn_extract, label="Dynamic")
   axs[3].plot(loc, p_extract, label="Thermal")
   axs[3].vlines(loc[imagnetopause_], pmin, pmax, colors="r", linestyle="dashed", alpha=0.5)

   axs[4].plot(loc, bz, label="Bz")
   axs[4].hlines(0.0, loc[1], loc[end], colors="k", linestyle="dashed", alpha=0.2)
   axs[4].vlines(loc[imagnetopause_], bmin, bmax, colors="r", linestyle="dashed", alpha=0.5)
   ...
   axs[end].set_xlabel("x [Re]", fontsize=14)
   axs[1].set_ylabel("Density [amu/cc]", fontsize=14)
   axs[2].set_ylabel("Velocity [km/s]", fontsize=14)
   axs[3].set_ylabel("Pressure [nPa]", fontsize=14)
   axs[4].set_ylabel("Magnetic field [nT]", fontsize=14)

   str_title = @sprintf "t= %4.1fs" meta.time
   axs[1].set_title(str_title, fontsize=14)
